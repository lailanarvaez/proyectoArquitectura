angular.module('controller', [])
    ...
    .controller('configuracionCtrl', ['$scope', '$http', '$log', '$timeout', '$location', function ($scope, $http, $log, $timeout, $location) {

        $scope.user = {};
        $scope.notification = {};
        $scope.usuario = {};
        $scope.removeData = function ($params) {
            var cnfrm = confirm("Estas seguro de desactivar tu cuenta?");
            if (cnfrm) {
                $http.post('./model/removeData.php', { 'id': $params })
                    .success(function (data) {
                        $scope.notification.success = true;
                        $scope.notification.message = "Podras volverla activar cuando desees!";
                        $timeout(function () {
                            $scope.notification = {};
                            // change the path
                            $location.path('/');
